    ...
    return (
      <div className="max-w-2xl space-y-6">
        <div className="flex items-center justify-between">
          <h1 className="font-display text-2xl text-navy-800">{isNew ? 'New FAQ' : 'Edit FAQ'}</h1>
        ...
        <Card>
          <Field label="Question">
            <Input value={editing.question} onChange={(e) => setEditing({ ...editing, question: e.target.value })} placeholder="What programs do you offer?" />
          </Field>
          <Field label="Answer" className="mt-4">
            <Textarea rows={4} value={editing.answer} onChange={(e) => setEditing({ ...editing, answer: e.target.value })} placeholder="We offer BSc. CSIT, BCA and BBS, all affiliated with Tribhuvan University." />
          </Field>
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-navy-500 mt-1">
            These show up in the "Chat with Admissions" widget — visitors tap a question and get this answer instantly, no admin needed.
          </p>
        </div>
        <Button onClick={() => { setEditing(empty()); setIsNew(true); setError(''); }}><Plus size={16} /> New FAQ</Button>
      </div>

      {loading ? <p className="text-sm text-navy-400">Loading…</p> : faqs.length === 0 ? (
        <Card><EmptyState text="No FAQs yet. Add a few common questions to power the instant-answer chat widget." /></Card>
      ) : (
            ...
            <Card key={f._id}>
              <div className="flex items-start justify-between gap-4">
                ...
                <div className="flex gap-2 shrink-0">
                  <IconButton onClick={() => { setEditing(f); setIsNew(false); setError(''); }}><Pencil size={16} /></IconButton>
