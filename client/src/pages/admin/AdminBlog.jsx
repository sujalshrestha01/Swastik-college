...
export default function AdminBlog() {
  ...
  const fetchPosts = async () => {
    ...
    try {
      ...
      setMessage({ type: 'danger', text: 'Failed to load blog posts from server.' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this post?')) {
      try {
        ...
        setMessage({ type: 'success', text: 'Blog post deleted from database.' });
      } catch (err) {
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      if (isExisting) {
        ...
        setPosts((prev) => prev.map((p) => ((p._id || p.id) === targetId ? updated : p)));
        setMessage({ type: 'success', text: 'Blog post updated successfully!' });
      } else {
        const newPost = await blogAdmin.create(currentPost);
        setPosts((prev) => [newPost, ...prev]);
        setMessage({ type: 'success', text: 'Blog post created and saved to database!' });
      }

      setIsEditing(false);
    } catch (err) {
      setMessage({ type: 'danger', text: 'Error saving blog post to database.' });
    }
  };
  ...
  return (
    <div className="space-y-6 max-w-5xl">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="font-display text-2xl text-navy-800 dark:text-paper font-bold">Blog Management</h1>
          <p className="text-sm text-navy-500">Create, edit, or remove articles from the database.</p>
        </div>
        {!isEditing && (
      ...
      {isEditing ? (
        <Card title={currentPost._id || currentPost.id ? 'Edit Article' : 'New Article'}>
        ...
        <Card>
          ...
          {loading ? (
            <p className="py-4 text-sm text-navy-500">Loading articles from database...</p>
          ) : filteredPosts.length === 0 ? (
            <p className="py-4 text-sm text-navy-500">No articles found in database.</p>
          ) : (
            <div className="divide-y divide-navy-100">
              {filteredPosts.map((post) => {
                ...
                return (
                  <div key={postId} className="py-4 flex items-center justify-between gap-4">
                    <div className="flex items-center gap-3 min-w-0">
                    {post.imageUrl && (
                      <img src={resolveImageUrl(post.imageUrl)} alt="" className="w-14 h-14 rounded-lg object-cover shrink-0" />
                    )}
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className="text-xs text-navy-400">{postDate}</span>
                      </div>
                      <h3 className="font-semibold text-navy-800">{post.title}</h3>
                      <p className="text-xs text-navy-500">By {post.author}</p>
                    </div>
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <IconButton variant="danger" onClick={() => handleDelete(postId)}>
                        <Trash2 size={16} />
                      </IconButton>
                    </div>
                  </div>
