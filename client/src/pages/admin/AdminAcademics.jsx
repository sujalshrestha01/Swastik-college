      ...
      <div className="max-w-2xl space-y-6">
        <div className="flex items-center justify-between">
          <h1 className="font-display text-2xl text-navy-800">
            {isNew ? "New Certification Course" : "Edit Course"}
          </h1>
        ...
        <Card>
          <Field label="Logo / icon" className="mb-4">
            <ImageUpload
              ...
              shape="square"
              hint="A small square logo works best"
            />
          </Field>
  ...
  if (editing && editingType === "workshop") {
    return (
      <div className="max-w-2xl space-y-6">
        <div className="flex items-center justify-between">
          <h1 className="font-display text-2xl text-navy-800">
            {isNew ? "New Live Workshop" : "Edit Workshop"}
          </h1>
          <div className="flex gap-2">
            <Button variant="secondary" onClick={() => setEditing(null)}>
              <X size={16} /> Cancel
        ...
        <Card>
          ...
          <Field label="Agenda highlights (one per line)" className="mt-4">
            <Textarea
              ...
              placeholder={
                "Interactive Rebase & Branching\nResolving Merge Conflicts"
              }
            />
          </Field>
          <Field
            label="Enrollment link (Google Form or any registration URL)"
            className="mt-4"
          >
            <Input
              value={editing.enrollUrl}
              onChange={(e) =>
                setEditing({ ...editing, enrollUrl: e.target.value })
              }
              placeholder="https://forms.google.com/..."
            />
          </Field>
          <p className="text-xs text-navy-400 mt-2">
            This is the link visitors are sent to when they click "Enroll Now" —
            update it any time a new workshop opens.
          </p>
        </Card>
      </div>
    );
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-navy-500 mt-1">
          Manage the certification courses and live workshops shown on the
      ...
      {tab === "courses" && (
        <div className="space-y-4">
          <div className="flex justify-end">
            <Button onClick={() => startEdit("course", emptyCourse(), true)}>
          ...
          {loading ? (
            ...
            <Card>
              <EmptyState text="No certification courses yet." />
            </Card>
          ) : (
                ...
                <Card key={c._id}>
                  <div className="flex items-center justify-between gap-4">
                    <div className="flex items-center gap-3 min-w-0">
                      {c.logoUrl && (
                        <img
                          src={resolveImageUrl(c.logoUrl)}
                          alt=""
                          className="w-10 h-10 rounded-lg object-contain bg-navy-50 shrink-0"
                        />
                      )}
                    ...
                    <div className="flex gap-2 shrink-0">
                      <IconButton onClick={() => startEdit("course", c, false)}>
                        <Pencil size={16} />
                      </IconButton>
                      <IconButton
                        variant="danger"
      ...
      {tab === "workshops" && (
        <div className="space-y-4">
          <div className="flex justify-end">
            <Button
              onClick={() => startEdit("workshop", emptyWorkshop(), true)}
            >
              ...
              {workshops.map((w) => (
                <Card key={w._id}>
                  <div className="flex items-center justify-between gap-4">
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <div className="min-w-0">
                        <div className="flex items-center gap-2">
                          ...
                          {!w.isActive && (
                            <span className="text-[10px] font-semibold bg-navy-100 text-navy-500 px-2 py-0.5 rounded-full">
                              Hidden
                            </span>
                          )}
                        </div>
                    ...
                    <div className="flex gap-2 shrink-0">
                      <IconButton
                        onClick={() => startEdit("workshop", w, false)}
                      >
                        <Pencil size={16} />
                      </IconButton>
                      <IconButton
                        variant="danger"
                        onClick={() => handleDelete("workshop", w._id)}
                      >
                        <Trash2 size={16} />
                      </IconButton>
                    </div>
