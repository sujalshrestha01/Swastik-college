...
export default function AdminGallery() {
  ...
  async function handleDelete(id) {
    if (!confirm("Delete this whole album? All photos in it will be removed."))
      return;
      ...
      <div className="max-w-3xl space-y-6">
        <div className="flex items-center justify-between">
          <h1 className="font-display text-2xl text-navy-800">
            {isNew ? "New Gallery Album" : "Edit Gallery Album"}
          </h1>
        ...
        <Card
          title="Photos"
          description="Upload directly from your device. Click the star on a photo to make it the album's cover/thumbnail."
        >
          <MultiImageUpload
            ...
            onThumbnailChange={(id) =>
              setEditing((prev) => ({ ...prev, thumbnailId: id }))
            }
          />
        </Card>
        <Card>
          ...
          <label className="mt-4 flex items-center gap-2 text-sm text-navy-700 cursor-pointer">
            <input
              ...
              onChange={(e) =>
                setEditing({ ...editing, featuredOnHome: e.target.checked })
              }
            />
            Show in "The Swastik Experience" on the home page
          </label>
          <p className="text-xs text-navy-400 mt-1">
            Up to 4 featured albums show there, using the same "Display order"
            field above (lowest first).
          </p>
        </Card>
      </div>
    );
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-navy-500 mt-1">
            Each album can hold multiple photos — pick one as the cover shown on
            the public grid.
          </p>
        </div>
        <Button
      ...
      {loading ? (
        ...
        <Card>
          <EmptyState text="No gallery albums yet. Create one to showcase campus photos." />
        </Card>
      ) : (
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {items.map((event) => (
            <div
              key={event._id}
              className="relative rounded-2xl overflow-hidden border border-navy-100 bg-white group"
            >
              ...
              <div className="p-3">
                <p className="text-xs text-marigold-600 font-semibold">
                  {event.category}
                </p>
