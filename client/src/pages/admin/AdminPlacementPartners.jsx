...
export default function AdminPlacementPartners() {
    ...
    return (
      <div className="max-w-xl space-y-6">
        <div className="flex items-center justify-between">
          <h1 className="font-display text-2xl text-navy-800">{isNew ? 'New Placement Partner' : 'Edit Placement Partner'}</h1>
        ...
        <Card>
          <div className="space-y-4">
            ...
            <Field label="Logo"><ImageUpload value={editing.logoUrl} onChange={(url) => setEditing({ ...editing, logoUrl: url })} /></Field>
            <Field label="Website URL" hint="Optional — makes the logo clickable"><Input value={editing.websiteUrl} onChange={(e) => setEditing({ ...editing, websiteUrl: e.target.value })} placeholder="https://" /></Field>
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="font-display text-2xl text-navy-800">Placement Partners</h1>
        <Button onClick={() => { setEditing(empty()); setIsNew(true); }}><Plus size={16} /> New Partner</Button>
      </div>
      {loading ? <p className="text-sm text-navy-400">Loading…</p> : items.length === 0 ? (
        <Card><EmptyState text="No placement partners yet." /></Card>
      ) : (
            ...
            <Card key={p._id}>
              <div className="flex items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                  {p.logoUrl && (
                     <img src={resolveImageUrl(p.logoUrl)} alt={p.name} className="h-10 w-auto max-w-[100px] object-contain bg-white rounded border border-navy-100 p-1" />
                 )}
                  <p className="text-sm font-semibold text-navy-800">{p.name}</p>
                </div>
                <div className="flex gap-2 shrink-0">
                  <IconButton onClick={() => { setEditing(p); setIsNew(false); }}><Pencil size={16} /></IconButton>
                  <IconButton variant="danger" onClick={() => handleDelete(p._id)}><Trash2 size={16} /></IconButton>
                </div>
              </div>
            </Card>
          ))}
