    ...
    return (
      <div className="max-w-xl space-y-6">
        <div className="flex items-center justify-between">
          <h1 className="font-display text-2xl text-navy-800">{isNew ? 'New Testimonial' : 'Edit Testimonial'}</h1>
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Button onClick={() => { setEditing(empty()); setIsNew(true); }}><Plus size={16} /> New Testimonial</Button>
      </div>
      {loading ? <p className="text-sm text-navy-400">Loading…</p> : items.length === 0 ? (
        <Card><EmptyState text="No testimonials yet." /></Card>
      ) : (
            ...
            <Card key={t._id}>
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <p className="text-sm font-semibold text-navy-800 mt-2">{t.name}</p>
                  <p className="text-xs text-navy-400">{t.role}</p>
                </div>
                <div className="flex gap-2 shrink-0">
                  <IconButton onClick={() => { setEditing(t); setIsNew(false); }}><Pencil size={16} /></IconButton>
                  <IconButton variant="danger" onClick={() => handleDelete(t._id)}><Trash2 size={16} /></IconButton>
                </div>
              </div>
            </Card>
          ))}
