        ...
        <div className="lg:col-span-3">
          {status === 'sent' ? (
            <div className="border border-teal-200 dark:border-teal-500/30 bg-teal-50 dark:bg-teal-500/10 rounded-xl p-8 text-center">
              <CheckCircle2 className="mx-auto text-teal-600 dark:text-teal-400 mb-3" size={32} />
              <h3 className="font-display text-xl text-navy dark:text-paper mb-1">Inquiry sent</h3>
              <p className="text-sm text-navy-400 dark:text-navy-200">
                Thanks, {form.name.split(' ')[0] || 'there'} — the admissions office will follow up by email.
              </p>
            </div>
          ) : (
            <form onSubmit={handleSubmit} className="space-y-5 bg-white dark:bg-navy-800 border border-navy-100 dark:border-navy-700 rounded-xl p-6 sm:p-8">
              <div className="grid sm:grid-cols-2 gap-5">
                <div>
                  <label className="block text-xs font-medium text-navy-500 dark:text-navy-200 mb-1.5">Full name</label>
                  <input
                    required
                    value={form.name}
                ...
                <div>
                  <label className="block text-xs font-medium text-navy-500 dark:text-navy-200 mb-1.5">Email</label>
                  <input
                    required
                    type="email"
                    value={form.email}
              ...
              <div className="grid sm:grid-cols-2 gap-5">
                <div>
                  <label className="block text-xs font-medium text-navy-500 dark:text-navy-200 mb-1.5">Phone</label>
                  <input
                    value={form.phone}
                ...
                <div>
                  <label className="block text-xs font-medium text-navy-500 dark:text-navy-200 mb-1.5">Program of interest</label>
                  <select
                    value={form.program}
              ...
              <div>
                <label className="block text-xs font-medium text-navy-500 dark:text-navy-200 mb-1.5">Message</label>
                <textarea
                  required
                  rows={4}
                  value={form.message}
