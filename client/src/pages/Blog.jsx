...
import { Calendar, User, Tag, ArrowRight, ArrowLeft, Search } from 'lucide-react';
...
function BlogDetail({ slug }) {
  ...
  if (loading) return <div className="py-20 text-center text-navy-400 text-sm">Loading article…</div>;
  if (!post) {
    return (
      <div className="py-20 text-center space-y-3">
        <p className="text-navy-500">This article couldn't be found.</p>
        <Link to="/blog" className="text-marigold-600 font-semibold hover:underline inline-flex items-center gap-1">
  ...
  return (
    <article className="max-w-3xl mx-auto py-12 px-4 sm:px-6">
      <Link to="/blog" className="text-sm text-navy-500 hover:text-marigold-600 inline-flex items-center gap-1 mb-6">
        <ArrowLeft size={14} /> Back to Blog
      </Link>
      {post.imageUrl && (
        <img src={resolveImageUrl(post.imageUrl)} alt={post.title} className="w-full h-72 object-cover rounded-2xl mb-6" />
      )}
      <div className="flex items-center gap-4 text-xs text-navy-500 dark:text-navy-400 mb-3">
        {post.category && <span className="inline-flex items-center gap-1"><Tag size={13} /> {post.category}</span>}
        <span className="inline-flex items-center gap-1">
          <Calendar size={13} /> {new Date(post.createdAt || Date.now()).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}
        </span>
        {post.author && <span className="inline-flex items-center gap-1"><User size={13} /> {post.author}</span>}
      </div>
      <h1 className="font-display text-3xl sm:text-4xl font-bold text-navy-900 dark:text-paper mb-6">{post.title}</h1>
    ...
    <div className="py-12 px-4 sm:px-6 max-w-7xl mx-auto space-y-10">
      ...
      <div className="text-center space-y-3">
        ...
        <p className="text-navy-600 dark:text-navy-300 max-w-2xl mx-auto text-sm sm:text-base">
          Articles, tech guides, campus news, and project showcases written by our faculty and students.
        </p>
      </div>
      </Section>
      ...
      {/* Blog Cards Grid */}
      {loading ? (
        <div className="text-center py-16 text-navy-400 text-sm">Loading articles…</div>
