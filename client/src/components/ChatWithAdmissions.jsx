...
import { MessageCircle, X, Send, ArrowLeft, CheckCircle2, Bot, User } from 'lucide-react';
...
export default function ChatWithAdmissions({ onClose }) {
  ...
  function handleAsk(faq) {
    setThread((prev) => [...prev, { role: 'user', text: faq.question }, { role: 'bot', text: faq.answer }]);
  }

  async function handleFormSubmit(e) {
    e.preventDefault();
    setFormStatus('sending');
    await submitContactForm({ ...form, message: `[Live chat] ${form.message}` });
    setFormStatus('sent');
  }
      ...
      <div className="bg-navy-800 dark:bg-navy-900 text-white px-4 py-3.5 flex items-center justify-between shrink-0">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-[11px] text-navy-300 leading-tight">Usually answers instantly</p>
          </div>
        </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3 bg-paper dark:bg-navy-900/40">
        {mode === 'faq' ? (
          <>
            <div className="flex gap-2 items-start">
              ...
              <p className="text-sm bg-white dark:bg-navy-800 border border-navy-100 dark:border-navy-700 rounded-2xl rounded-tl-sm px-3.5 py-2.5 text-navy-700 dark:text-navy-100">
                Hi! Tap a question below for an instant answer, or message admissions directly if you don't see what you need.
              </p>
            </div>
              ...
              <div key={i} className={`flex gap-2 items-start ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                <span
                  className={`w-6 h-6 rounded-full flex items-center justify-center shrink-0 mt-0.5 ${
                    msg.role === 'user' ? 'bg-marigold-400 text-navy-900' : 'bg-navy-100 dark:bg-navy-700 text-navy-500'
                  }`}
                >
            ...
            {loadingFaqs ? (
              <p className="text-xs text-navy-400 text-center py-4">Loading questions…</p>
            ) : faqs.length === 0 ? (
              <p className="text-xs text-navy-400 text-center py-4">No FAQs added yet.</p>
            ) : (
          ...
          <div className="text-center py-6">
            <CheckCircle2 className="mx-auto text-teal-600 dark:text-teal-400 mb-2" size={30} />
            <p className="text-sm font-semibold text-navy-800 dark:text-paper">Message sent!</p>
            <p className="text-xs text-navy-400 mt-1">Admissions will follow up by email soon.</p>
          </div>
        ) : (
          <form onSubmit={handleFormSubmit} className="space-y-3">
            <button type="button" onClick={() => setMode('faq')} className="text-xs text-navy-400 hover:text-navy-600 inline-flex items-center gap-1 mb-1">
            ...
            >
              {formStatus === 'sending' ? 'Sending…' : (<><Send size={14} /> Send to Admissions</>)}
            </button>
          </form>
        )}
      </div>
