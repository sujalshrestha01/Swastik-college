    ...
    <div className={`bg-white rounded-2xl border border-navy-100 shadow-sm ${className}`}>
      {(title || description || action) && (
        <div className="px-5 py-4 border-b border-navy-100 flex items-center justify-between gap-4">
          <div>
            {title && <h2 className="font-display text-lg font-semibold text-navy-800">{title}</h2>}
...
export function Banner({ type = 'info', children }) {
  ...
  return <div className={`rounded-lg border px-4 py-2.5 text-sm ${styles[type] || styles.info}`}>{children}</div>;
}
