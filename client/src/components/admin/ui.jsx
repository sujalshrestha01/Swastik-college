    ...
    <div className="bg-white rounded-2xl border border-navy-100 shadow-sm">
      {(title || action) && (
        <div className="px-5 py-4 border-b border-navy-100 flex items-center justify-between gap-4">
          <div>
            {title && <h2 className="font-display text-lg text-navy-800">{title}</h2>}
            {description && <p className="text-sm text-navy-500 mt-0.5">{description}</p>}
          </div>
          {action}
        </div>
      )}
...
export function Field({ label, children, hint, className = '' }) {
  return (
    <label className={`block ${className}`}>
      <span className="block text-sm font-medium text-navy-700 mb-1">{label}</span>
      {children}
...
export function Button({ variant = 'primary', className = '', ...props }) {
  ...
  return (
    <button
      {...props}
      className={`inline-flex items-center gap-2 rounded-lg px-4 py-2 text-sm font-semibold transition-colors disabled:opacity-50 disabled:cursor-not-allowed ${variants[variant]} ${className}`}
    />
  );
}
...
export function IconButton({ variant = 'outline', className = '', ...props }) {
  ...
  return (
    <button
      {...props}
      className={`inline-flex items-center justify-center rounded-lg p-2 transition-colors ${variants[variant]} ${className}`}
    />
  );
}
...
export function Banner({ type = 'info', children }) {
  ...
  return <div className={`rounded-lg border px-4 py-2.5 text-sm ${styles[type]}`}>{children}</div>;
}
