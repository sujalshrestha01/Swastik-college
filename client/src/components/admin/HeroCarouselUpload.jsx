  ...
  return (
    <div className="space-y-3">
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
        {images.map((url, idx) => (
          <div key={url + idx} className="relative rounded-xl overflow-hidden border-2 border-navy-100 bg-navy-50 group">
            <img src={resolveImageUrl(url)} alt="" className="w-full h-28 object-cover" />
            ...
            <div className="absolute inset-0 bg-navy-950/0 group-hover:bg-navy-950/40 transition-colors flex items-center justify-center gap-1.5 opacity-0 group-hover:opacity-100">
              {idx > 0 && (
                <button type="button" onClick={() => moveImage(idx, -1)} className="p-1.5 rounded-full bg-white/90 text-navy-700 hover:bg-white" title="Move earlier">
                  <ArrowUp size={13} />
                </button>
              )}
              {idx < images.length - 1 && (
                <button type="button" onClick={() => moveImage(idx, 1)} className="p-1.5 rounded-full bg-white/90 text-navy-700 hover:bg-white" title="Move later">
                  <ArrowDown size={13} />
                </button>
              )}
              <button type="button" onClick={() => removeImage(idx)} className="p-1.5 rounded-full bg-white/90 text-red-600 hover:bg-white" title="Remove">
                <X size={13} />
              </button>
            </div>
          </div>
      ...
      <p className="text-xs text-navy-400">
        Add as many photos as you like. Use the arrows to reorder — the carousel plays them in this order. Leave empty to show a single static hero image instead.
      </p>
    </div>
  );
}