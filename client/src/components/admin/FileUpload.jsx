...
import { UploadCloud, X, Loader2, FileText, FileImage, ExternalLink } from 'lucide-react';
 ...
 */
export default function FileUpload({ value, onChange, label, hint, accept = 'application/pdf,image/*' }) {
      ...
      {value ? (
        ...
        <div
          ...
          className={`relative rounded-xl border-2 border-dashed transition-colors ${
            dragOver ? 'border-marigold-400 bg-marigold-50' : 'border-navy-200 bg-navy-50'
          }`}
        >
          <button
            type="button"
          ...
          >
            ...
            <span className="text-xs font-medium px-2 text-center">
              {uploading ? 'Uploading…' : 'Drag & drop a PDF or image here, or click to choose from your device'}
            </span>
          </button>
        </div>
      )}
