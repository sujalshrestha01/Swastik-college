    ...
    <footer className="bg-navy-900 text-navy-200 pt-16 pb-8 mt-16">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-10 pb-12 border-b border-navy-700">
            ...
            {socialLinks.length > 0 && (
              <div className="flex items-center gap-3 mt-5">
                {socialLinks.map(({ key, Icon, url }) => (
                  <a key={key} href={url} target="_blank" rel="noreferrer" aria-label={key} className="hover:text-marigold-300 transition-colors">
                    <Icon size={18} />
                  </a>
                ))}
              </div>
          ...
          <div>
            ...
            <ul className="space-y-3 text-sm">
              <li className="flex gap-2">
                <MapPin size={16} className="shrink-0 mt-0.5 text-marigold-300" />
                <a
              ...
              {settings.phone && (
                <li className="flex gap-2">
                  <Phone size={16} className="shrink-0 mt-0.5 text-marigold-300" />
                  <a href={`tel:${settings.phone}`} className="hover:text-marigold-300 transition-colors">{settings.phone}</a>
                </li>
              )}
              {settings.email && (
                <li className="flex gap-2">
                  <Mail size={16} className="shrink-0 mt-0.5 text-marigold-300" />
                  <a href={`mailto:${settings.email}`} className="hover:text-marigold-300 transition-colors">
                    {settings.email}
                  </a>
                </li>
              )}
          ...
          <div>
            <h4 className="text-paper font-medium text-sm mb-4">Office Hours</h4>
        ...
        <p className="text-xs text-navy-400 text-center pt-6">
          © {new Date().getFullYear()} {settings.collegeName}. All rights reserved.
        </p>
      </div>
    </footer>
  );
