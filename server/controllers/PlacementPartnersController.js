...
export async function listPlacementPartners(req, res) {
  try {
    ...
    res.status(500).json({ message: 'Failed to fetch placement partners', error: err.message });
  }
}
...
export async function createPlacementPartner(req, res) {
  try {
    ...
    res.status(400).json({ message: 'Failed to create placement partner', error: err.message });
  }
}

export async function updatePlacementPartner(req, res) {
  try {
    const partner = await PlacementPartner.findByIdAndUpdate(req.params.id, req.body, {
      new: true,
      runValidators: true,
    });
    if (!partner) return res.status(404).json({ message: 'Placement partner not found' });
    res.json(partner);
  } catch (err) {
    res.status(400).json({ message: 'Failed to update placement partner', error: err.message });
  }
}
...
export async function deletePlacementPartner(req, res) {
  try {
    ...
    res.status(500).json({ message: 'Failed to delete placement partner', error: err.message });
  }
}