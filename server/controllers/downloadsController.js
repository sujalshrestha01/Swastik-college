...
export async function getDownload(req, res) {
  try {
    ...
    res.status(500).json({ message: 'Failed to fetch download', error: err.message });
  }
}
...
export async function createDownload(req, res) {
  try {
    ...
    res.status(400).json({ message: 'Failed to create download', error: err.message });
  }
}

export async function updateDownload(req, res) {
  try {
    const item = await Download.findByIdAndUpdate(req.params.id, req.body, { new: true, runValidators: true });
    if (!item) return res.status(404).json({ message: 'Download not found' });
    res.json(item);
  } catch (err) {
    res.status(400).json({ message: 'Failed to update download', error: err.message });
  }
}
...
export async function deleteDownload(req, res) {
  try {
    ...
    res.status(500).json({ message: 'Failed to delete download', error: err.message });
  }
}
