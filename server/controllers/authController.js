...
export async function login(req, res) {
  try {
    ...
    if (admin.status === 'pending' || !admin.password) {
      return res.status(401).json({ message: 'Invite not yet accepted. Check your invite link.' });
    }
...
export async function inviteAdmin(req, res) {
  try {
    if (req.admin.role !== 'superadmin') {
      return res.status(403).json({ message: 'Only a superadmin can invite new admins' });
    }
    ...
    res.status(201).json({
      ...
      admin: { id: invited._id, name: invited.name, email: invited.email, role: invited.role },
      inviteLink,
    });
  } catch (err) {
